...
;findPas(){
  ...
  login() {
    NetInfo.fetch().done((reach) => {
      if(reach=="NONE"){
        Toast.show("当前设备处于离线状态！请联网后重试","short");
      }
    });
    if (this.state.phonenumber == "" || this.state.password == "") {
      Toast.show("请检查用户名和密码","short");
    }
    else {
      ...
      api.User.userLogin(this.state.phonenumber, this.state.password)
        .then((resData)=> {
          ...
          if (resData.Type == 0) {
            Toast.show("登录失败，请检查用户名和密码！","short");
          ...
          <View>
            <View
              ...
              <TextInput
                ...
                placeholder="输入手机或邮箱"
                textAlignVertical={'center'} textAlign={'start'} style={styles.TextInputs}
